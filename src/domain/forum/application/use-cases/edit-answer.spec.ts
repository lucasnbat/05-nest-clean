...
describe('Edit Answer', () => {
  ...
  it('should be able to edit a question sync new and removed attachments when editing an answer', async () => {
    const newAnswer = makeAnswer(
      {
        authorId: new UniqueEntityID('author-1'),
      },
    ...
    // salva no repositorio (lembre: metodo "save" é para edição, salvar no repo
    // é modo de dizer quando usamos o create)
    inMemoryAnswersRepositoryInstance.create(newAnswer)
    ...
    expect(inMemoryAnswerAttachmentsRepositoryInstance.items).toHaveLength(2)
    expect(inMemoryAnswerAttachmentsRepositoryInstance.items).toEqual(
